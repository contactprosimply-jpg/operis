...
const nav = [
  ...
  { href: '/suppliers', label: 'Fournisseurs', icon: (a: boolean) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={a ? 2 : 1.6} width="20" height="20"><rect x="1" y="3" width="15" height="13" rx="1"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg> },
  ...
  const AccountPanel = ({ mobile = false }: { mobile?: boolean }) => (
    ...
    >
      ...
      <div style={{ padding: '8px 0' }}>
        <Link href="/compte" onClick={() => setShowAccountPanel(false)} style={{ width: '100%', padding: '9px 16px', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', fontSize: 13, color: '#8b92a5', display: 'flex', alignItems: 'center', gap: 10, transition: 'background 0.1s', fontFamily: 'DM Sans, system-ui', textDecoration: 'none' }}
          onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.05)'}
          onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = 'transparent'}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" width="16" height="16"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
          Mon compte
        </Link>
    ...
    <>
          ...
          {visibleNav.map(item => {
            ...
            return (
              <div key={item.href} className="nav-item" style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
                <Link href={item.href} prefetch={true} aria-label={item.label} aria-current={active ? 'page' : undefined} data-tour={`nav-${item.href.slice(1)}`} style={{
      ...
      <nav className="mobile-bottom-bar">
        {visibleNav.map(item => {
          ...
          const isMail = item.href === '/mail'
          return (
            <Link key={item.href} href={item.href} prefetch={true} aria-label={item.label} aria-current={active ? 'page' : undefined} data-tour={`nav-${item.href.slice(1)}`} className="mobile-nav-item" style={{
              color: active ? '#6ba3f9' : 'var(--text-muted)',
              background: active ? 'var(--accent-soft)' : 'transparent',
            }}>
              {item.icon(active)}
